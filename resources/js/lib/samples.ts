...
export const listSamples = async (
	...
	const url = buildUrl(passes.samples.index().url, params as Record<string, string>);
	...
	return (await response.json()) as { data: PassTypeSample[] } | PassTypeSample[];
};
...
export const deleteSample = async (sampleId: string) => {
	const response = await fetch(passes.samples.destroy({ sample: sampleId }).url, {
		method: 'DELETE',
		headers: {
			Accept: 'application/json',
			'X-CSRF-TOKEN': getCsrfToken(),
...
export const listMediaAssets = async (
	...
	const url = buildUrl(passes.media.assets.index().url, params as Record<string, string>);
	const response = await fetch(url, {
		headers: {
			Accept: 'application/json',
		},
	...
	return (await response.json()) as { data: MediaLibraryAsset[] } | MediaLibraryAsset[];
};
...
export const deleteMediaAsset = async (assetId: string) => {
	const response = await fetch(passes.media.assets.destroy({ asset: assetId }).url, {
		method: 'DELETE',
		headers: {
			Accept: 'application/json',
			'X-CSRF-TOKEN': getCsrfToken(),
...
export const getPassTypeFieldMap = async (passType: PassType, platform: PassPlatform) => {
...
export const buildImagesFromSample = (sample: PassTypeSample, platform: PassPlatform): PassImages => {
	...
	IMAGE_SLOTS.forEach((slot) => {
		...
		const asset = typeof entry === 'string' ? { path: entry, url: entry } : entry;
...
export const collectSampleImagePayload = (images: PassImages, platform: PassPlatform) => {
...
export const hasAllSampleImageSlots = (payload: Partial<Record<PassImageSlot, string>>) => {
